...
async function getHttpClient(options) {
    ...
    if (ImpitClass === null) {
        try {
            ...
            if (!fallbackWarningShown) {
                console.warn("\x1b[33m⚠️  camoufox-js: impit native module not available.\n" +
    ...
    if (options.proxyUrl && !fallbackWarningShown) {
        console.warn("\x1b[33m⚠️  camoufox-js: Proxy ignored for IP detection (impit unavailable)\x1b[0m");
    }
    return {
        async fetch(url) {
            return fetch(url, {
